...
substitution.  Honda and Yoshida \cite{DBLP:conf/popl/HondaY94,DBLP:journals/tcs/Yoshida02} describe an elimination algorithm
...
RHO combinators (see below), \NYnote{It is too long to say
RHO combinator calculus I propose you introduce the abbrev like
CC$\rho$ as macros and replace all occurrences of RHO combinator
calculus. This saves the space.} RHO combinators can see
...
parameterized in a theory of names, we instantiate her calculus
\NYnote{Please replace her calculus or she to their or them -- please
  check all sections and Appendix carefully.} with
...
the mapping with two name constructors $x^r$ and $x^l$ as:
\begin{equation*}
...
\begin{example}
\NYnote{Please put some example. You can look at POPL paper where we
  demonstrate this main translation as small key examples. Do not
  worry about the space.}
\end{example}
Note that all $\new$-binding is now interpreted, as in Wischik's
global $\pi$-calculus, as an input \cite{globalpi}:
the alpha-equivalence of $\new$ is handled by input-binding,
...
\NYnote{We need some short description of this subsection}
