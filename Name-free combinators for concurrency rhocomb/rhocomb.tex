...
\noindent
  ...
  but still make heavy use of the $\mathsf{new}$ operator. Meanwhile
  ...
  entirely name-free account of mobile concurrency that does not
  suffer a centralized resource ensuring global uniqueness of names.
\end{abstract}
...
the {\rhoc}. \cite{DBLP:conf/concur/Milner92}
\cite{DBLP:journals/entcs/MeredithR05} However, all such models
currently proposed make heavy use of nominal phenomena. Even Honda and
Yoshida's combinatory model makes heavy use of nominal
phenomena. \cite{DBLP:conf/popl/HondaY94}
\cite{DBLP:journals/tcs/Yoshida02} While our algorithm can handle
...
results. It is remarkable that a single feature, i.e. reflection, suffices
...
``atoms'' constitute a source of risk. Indeed, it is precisely the
foundational theoretical questions raised by the Fraenkel-Mostowski
set theoretic approach that motivated Meredith and Radestock's the
investigation of the {\rhoc}: what better place to look for the source
of ``atoms'' than in the reification of theory of computation
requiring them?
...
substitution.  Honda and Yoshida \cite{DBLP:conf/popl/HondaY94}
...
into a higher-order {\pic} and implement $\new$ and replication in
terms of reflection.  This work presents a fusion of those ideas: a
...
extend Honda and Yoshida's original minimality and separation
results. \cite{DBLP:journals/tcs/Yoshida02} Specifically, this
calculus uses neither the $\mathsf{new}$ operator nor replication, but
only the original combinators together with a single combinator for
reflection, and yet capture the full power of the {\pic}. Thus, it is
possible extend the original results by considering combinator sets
that include or exclude the reflection combinator. In light of these
...
calculus was not found sooner. In point of fact, the initial
implementations of {\rhoc} in OCaml, Haskell, and Scala all used
exactly this fixed point formulation to define the syntax for the
{\rhoc}. The fact that the code type-checks and that the types are
inhabited provides some assurance of the soundness of the
construction.
...
(\cite{Quine59} \cite{Quine60}), who was also a pioneer of reflective
...
\begin{mathpar}
  \inferrule*[lab=Out-barb]{y \in {\mathcal N}, \; x \nameeq y}{\mathsf{m}(x-) \downarrow_{\mathcal{N}} x}
  \and
\inferrule*[lab=Par-barb]{\mbox{$P\downarrow_{\mathcal{N}} x$ or $Q\downarrow_{\mathcal N} x$}}{\binpar{P}{Q} \downarrow_{\mathcal{N}} x}
 \end{mathpar}
