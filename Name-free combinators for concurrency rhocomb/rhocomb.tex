...
{\rhocc} (see below), {\rhocc} combinators can see
...
translation will be given in terms of a function \[\meaningof{-}_2( -,
...
\begin{example}
  ...
  \[\binpar{\prefix{y}{x}{0}}{(\mathsf{new}\;u)x!(u)}\].
  The {\ccomb} translation will yield (see \ref{appendix:ccomb} for details)
\begin{eqnarray*}
  \meaningof{\binpar{\prefix{y}{x}{0}}{(\mathsf{new}\;u)x!(u)}} & = & \binpar{\meaningof{\prefix{y}{x}{0}}}{(\meaningof{\mathsf{new}\;u)x!(u)}} \\
  & = & \binpar{\meaningof{\prefixst{y}{x}{0}}}{(\mathsf{new}\;u)\meaningof{x!(u)}} \\
...
necessary to invocake structural equivalence, yielding
$(\mathsf{new}\;u)\binpar{\mathsf{k}(x)}{\mathsf{m}(x,u)}$. This step
corresponds exactly to the forwarding of $n^{r}$ along $p^{r}$ in the
reduction of
$\binpar{\mathsf{k}(x)}{\binpar{\mathsf{fw}(p^{r},x)}{\mathsf{m}(p^{r},n^{r})}}$
\end{example}
