...
what is found in Meredith and Radestock, to build a proof theory for
an interactive rewrite system. The proof theory enjoys a cut
elimination property that corresponds exactly with the notion of
...
\begin{itemize}
  ...
  \item $\mathsf{K}_{[]}^{l} : \chi_{T}^{2} \rightarrow \chi_{T}$
  \item $\mathsf{K}_{[]}^{r} : T x \chi_{T} \rightarrow \chi_{T}$
  ...
  \item $@([]!,t) = t$
  \item $@(![],t) = t$
  \item $@(\mathsf{K}_{[]}^{l} (\chi, t)!, u) = \mathsf{K}(@_{[]}(\chi, u), t)$
  \item  $@(!\mathsf{K}_{[]}^{r} (t, \chi), u) = \mathsf{K}(t, @_{[]}(\chi, u))$
\end{itemize}

As mentioned before, the basic notion of arrow is given by

\begin{eqnarray*}
  \tau \rhd_{\mathsf{K}} \tau’ & \triangleq  & \{ u | \exists t. u = !\mathsf{K}_{[]}^{r}(t, []),\forall u’:\tau. (\exists \rho : u@u’\rightarrow v) \Rightarrow v:\tau’ \} \\
  \tau’\lhd_{\mathsf{K}} \tau & \triangleq  & \{ t | \exists u. t = \mathsf{K}_{[]}^{l}([], u)!,\forall t’:\tau. (\exists \rho : t@t’\rightarrow v) \Rightarrow v:\tau’\}
\end{eqnarray*}

However, they can be generalized to

\begin{eqnarray*}
  \tau \rhd_{\mathsf{K}} \tau’ & \triangleq  & \{ u | \exists t, \chi. u = !\mathsf{K}_{[]}^{r}(t, \chi),\forall u’:\tau. (\exists \rho : u@u’\rightarrow v) \Rightarrow v:\tau’ \} \\
  \tau’\lhd_{\mathsf{K}} \tau & \triangleq  & \{ t | \exists u, \chi. t = \mathsf{K}_{[]}^{l}(\chi, u)!,\forall t’:\tau. (\exists \rho : t@t’\rightarrow v) \Rightarrow v:\tau’\}
\end{eqnarray*}
...
\subsection{Types}
\begin{mathpar}
  \inferrule* [lab=type] {} { \tau \bc  \top  | \tau \rhd \tau | \tau \lhd \tau | \mathsf{C} | \{\} | \tau \cup \tau | R\tau | \mathsf{K}(\tau,\tau) | \rho_{\otimes}(\tau,\tau) | \rho_{\bindnasrepma}(\tau,\tau) | \rho_{\sqcap}(\tau,\tau) | \rho_{\sqcup}(\tau,\tau) }
\end{mathpar}
...
\inferrule* [lab=$\rho_{\bindnasrepma \perp \otimes}$]{\Gamma \vdash \bindnasrepma_{\rho} (t_{1}, t_{2}) : \rho_{\bindnasrepma}(\tau_{1}^{\perp}, \tau_{2}^{\perp})  \Delta \vdash \mathsf{K}(t_{3}, t_{4}) : \rho_{\otimes}(\tau_{1}, \tau_{2}) }{\Gamma, \Delta \vdash R(\mathsf{K}(\mathsf{K}(t_{1}, t_{3}), \mathsf{K}(t_{2}, t_{4}))) : R(r_{\mathsf{K}})}
  
%% \tau \rhd \tau' = { u | \exists t,\chi. u = !\mathsf{K}[]r(t, \chi),\forall u’:\tau. (\exists \rho: u@u’ \rightarrow v) \Rightarrow v:\tau’ }
%% \tau\perp = \tau \rhd r_{\mathsf{K}} = { u | \exists t,\chi. u = !\mathsf{K}[]r(t, \chi),\forall u’:\tau. (\exists \rho: u@u’ \rightarrow v) \Rightarrow v:r_{\mathsf{K}} }
...
\inferrule* [lab=$\rho_{\sqcup \perp \sqcap}$]{\Gamma \vdash \sqcup_{\rho} (t_{1}, t_{2}) : \rho_{\sqcup}(\tau_{3} \rhd \tau_{1}^{\perp}, \tau_{3})  \Delta \vdash \mathsf{K}(t_{3}, t_{4}) : \rho_{\sqcap}(\tau_{2} \rhd \tau_{1}, \tau_{2})}{\Gamma, \Delta \vdash R(\mathsf{K}(\mathsf{K}(t_{1}, t_{2}), \mathsf{K}(t_{3}, t_{4}))) : R(r_{\mathsf{K}})}

\inferrule* [lab=$\rho_{\perp}$]{\Gamma \vdash t' : \tau^{\perp}, \Delta \vdash t : \tau }{\Gamma, \Delta \vdash R(\mathsf{K}(t',t)) : R(r_{\mathsf{K}})}
...
\begin{mathpar}
  ...
  \inferrule* [lab=context-cut-left] {\Gamma \vdash \mathsf{K}([],u)! : \tau' \lhd \tau,  \Delta \vdash t : \tau}{\Gamma, \Delta \vdash R(\mathsf{K}(t,u)) : R\tau'}

  \inferrule* [lab=context-cut-right] {\Gamma \vdash !\mathsf{K}(t,[]) : \tau \rhd \tau',  \Delta \vdash u : \tau} {\Gamma, \Delta \vdash R(\mathsf{K}(t,u)) : R\tau'}
\end{mathpar}
...
\begin{itemize}
  ...
  \item $\mathsf{K}_{[][]} : \chi^{\circ} \times \chi^{\bullet} \rightarrow \chi^{\circ\bullet}$
  \item $\mathsf{K}_{[]}^{l} : T \times \chi^{\circ\bullet} \rightarrow \chi^{\circ\bullet}$
  \item $\mathsf{K}_{[]}^{r} :  \chi^{\circ\bullet} \times T \rightarrow\chi^{\circ\bullet}$
  ...
  \item $@(\mathsf{K}_{[]}^{l} (\chi, t)!, u) = \mathsf{K}(@(\chi, u), t)$
  \item $@(!\mathsf{K}_{[]}^{r} (t, \chi), u) = \mathsf{K}(t, @(\chi, u))$
\end{itemize}
...
\begin{mathpar}
  ...
  \inferrule* [lab=$\rho_{\bindnasrepma\perp\otimes}$] {\vdash \Gamma, \bindnasrepma_{\rho} (t', u') : \rho_{\bindnasrepma}(\tau^{\perp}, \tau’^{\perp})  \vdash \mathsf{K}(t, u) : \rho_{\otimes}(\tau, \tau'), \Delta}{\vdash \Gamma, R(\mathsf{K}(\mathsf{K}(t’, t), \mathsf{K}(u’, u))) : R(r_{\mathsf{K}}), \Delta}
  ...
  \inferrule* [lab=$\rho_{\sqcap\perp\sqcup}$] {\vdash \Gamma, \sqcup\rho (t, u) : \rho\sqcup(\tau_{3} \rhd \tau_{1}^{\perp}, u:\tau_{3})  \vdash \mathsf{K}(t', u') : \rho\sqcap(\tau_{2} \rhd \tau_{1}, \tau_{2}), \Delta } {\vdash \Gamma, R(\mathsf{K}(\mathsf{K}( t, u), \mathsf{K}(t', u')) : R(r_{\mathsf{K}}), \Delta}

  \inferrule* [lab=$\rho_{\perp}$] {\vdash\Gamma,  t’ : \tau^{\perp}  \vdash t : \tau, \Delta }{\vdash \Gamma, R(\mathsf{K}(t’,t)) : R(r_{\mathsf{K}}), \Delta}
\end{mathpar}
...
\begin{mathpar}
  ...
  \inferrule* [lab=$\rho_{\otimes}\perp\lhd$] {\vdash \Gamma, \mathsf{K}([],u)! : \tau' \lhd \tau,  \vdash t : \tau, \Delta } {\vdash \Gamma, R(\mathsf{K}(t,u)) : R\tau', \Delta}

  \inferrule* [lab=$\rho_{\otimes}\perp\rhd$] {\vdash \Gamma, !\mathsf{K}(t,[]) : \tau \rhd \tau',  \Delta \vdash u : \tau}{\vdash \Gamma, R(\mathsf{K}(t,u)) : R\tau', \Delta}
\end{mathpar}
