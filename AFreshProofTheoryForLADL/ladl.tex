...
Specifically, we encode term calculi as Lawvere theories by starting with the theory
...
\begin{mathpar}
  ...
  \inferrule* [lab=$\rho_{\otimes}$]{\Gamma \vdash t : \tau  \quad \Delta \vdash u : \tau'}{\Gamma, \Delta \vdash \mathsf{K}(t, u) : \rho_{\otimes}(\tau, \tau')}
...
\inferrule* [lab=$\rho_{\bindnasrepma \perp \otimes}$]{\Gamma \vdash \bindnasrepma_{\rho} (t_{1}, t_{2}) : \rho_{\bindnasrepma}(\tau_{1} \rhd \tau_{i}^{\perp}, \tau_{2} \rhd \tau_{i}) \quad \Delta \vdash \mathsf{K}(t_{3}, t_{4}) : \rho_{\otimes}(\tau_{1}, \tau_{2}) }{\Gamma, \Delta \vdash \mathsf{R}(\mathsf{K}(\mathsf{R}(\mathsf{K}(t_{1}, t_{3})), \mathsf{R}(\mathsf{K}(t_{2}, t_{4})))) : \mathsf{r}_{\mathsf{K}}}
...
\inferrule* [lab=$\rho_{\perp}$]{\Gamma \vdash t' : \tau^{\perp}\quad \Delta \vdash t : \tau }{\Gamma, \Delta \vdash \mathsf{R}(\mathsf{K}(t',t)) : \mathsf{r}_{\mathsf{K}}}
...
\begin{mathpar}
  ...
  \inferrule* [lab=${\rightarrow}^{l}$]{\Gamma, \boxempty : \tau \vdash \mathsf{t}( \rho( t, \boxempty ) ) : \tau' }{\Gamma \vdash !\mathsf{K}( t, \boxempty ) : \tau \rhd \tau'} \\
  \inferrule* [lab=${\rightarrow}^{r}$]{\Gamma, \boxempty : \tau \vdash \mathsf{t}( \rho( \boxempty, u ) ) : \tau' }{\Gamma \vdash \mathsf{K}( \boxempty, u )! : \tau \lhd \tau'}
\end{mathpar}

\subsubsection{Weakening}

\begin{mathpar}
  \inferrule* [lab=weakening] {\Gamma \vdash \chi : \tau}{\boxempty : \tau', \Gamma \vdash \chi : \tau}
\end{mathpar}

\subsubsection{Left and right context formation and cut}

\begin{mathpar}
  \inferrule* [lab=$\lhd$] {\Gamma, \boxempty : \tau  \vdash \chi : \tau'} {\Gamma \vdash \chi ! : \tau' \lhd \tau}

  \inferrule* [lab=$\rhd$] {\Gamma, \boxempty : \tau  \vdash \chi : \tau'} {\Gamma \vdash !\chi : \tau \rhd \tau'} \\

  \inferrule* [lab=$\rho_{\boxempty \perp \lhd}$] {\Gamma \vdash \mathsf{K}(\boxempty,u)! : \tau' \lhd \tau \quad  \Delta \vdash t : \tau}{\Gamma, \Delta \vdash \mathsf{R}(\mathsf{K}(t,u)) : \tau'}

  \inferrule* [lab=$\rho_{\boxempty \perp \rhd}$] {\Gamma \vdash !\mathsf{K}(t,\boxempty) : \tau \rhd \tau'\quad  \Delta \vdash u : \tau} {\Gamma, \Delta \vdash \mathsf{R}(\mathsf{K}(t,u)) : \tau'}\\
  \inferrule* [lab=$\rho_{\sqcup \perp \sqcap}$]{\Gamma \vdash \sqcup_{\rho} (t_{1}, t_{2}) : \rho_{\sqcup}(\tau_{2} \rhd \tau_{1}^{\perp}, \tau_{2})  \quad \Delta \vdash \mathsf{K}(t_{3}, t_{4}) : \rho_{\sqcap}(\tau_{3} \rhd \tau_{1}, \tau_{3})}{\Gamma, \Delta \vdash \mathsf{R}(\mathsf{K}(\mathsf{R}(\mathsf{K}(t_{1}, t_{2})), \mathsf{R}(\mathsf{K}(t_{3}, t_{4})))) : \mathsf{r}_{\mathsf{K}}}
\end{mathpar}
